...
function displayClubDetails(club) {
  ...
  container.innerHTML = `
    <div class="print-header" style="display: none;">
      <h1 data-i18n="header.department_name"></h1>
      <div class="subtitle" data-i18n="header.department_name_en"></div>
      <div class="subtitle" style="font-weight: bold; margin-top: 3mm;" data-i18n="header.certificate_title"></div>
    </div>
    ...
    <div class="detail-card">
      <h2 data-i18n="form.club_information"></h2>
      <div class="info-row">
        <div><label data-i18n="form.reg_number"></label><p>${escapeHtml(
          club.reg_number,
        )}</p></div>
        <div><label data-i18n="form.registration_date"></label><p>${formatDate(
          club.registration_date,
        )}</p></div>
      </div>
      <div class="info-row">
        <div class="full-width"><label data-i18n="form.club_name"></label><p>${escapeHtml(
          club.name,
        )}</p></div>
      </div>
    </div>
    ...
    <div class="detail-card">
      <h2 data-i18n="form.location_information"></h2>
      <div class="info-row">
        <div><label data-i18n="form.district"></label><p>${escapeHtml(
          club.district_name || "-",
        )}</p></div>
        <div><label data-i18n="form.division"></label><p>${escapeHtml(
          club.division_name || "-",
        )}</p></div>
      </div>
      <div class="info-row">
        <div class="full-width"><label data-i18n="form.gn_division"></label><p>${escapeHtml(
    ...
    <div class="detail-card">
      <h2 data-i18n="form.chairman_information"></h2>
      <div class="info-row">
        <div><label data-i18n="form.chairman_name"></label><p>${escapeHtml(
          club.chairman_name,
        )}</p></div>
        <div><label data-i18n="form.chairman_phone"></label><p>${escapeHtml(
          club.chairman_phone,
        )}</p></div>
      </div>
      <div class="info-row">
        <div class="full-width"><label data-i18n="form.chairman_address"></label><p>${escapeHtml(
    ...
    <div class="detail-card">
      <h2 data-i18n="form.secretary_information"></h2>
      <div class="info-row">
        <div><label data-i18n="form.secretary_name"></label><p>${escapeHtml(
          club.secretary_name,
        )}</p></div>
        <div><label data-i18n="form.secretary_phone"></label><p>${escapeHtml(
          club.secretary_phone,
        )}</p></div>
      </div>
      <div class="info-row">
        <div class="full-width"><label data-i18n="form.secretary_address"></label><p>${escapeHtml(
    ...
    ${
      club.equipment && club.equipment.length > 0
        ? `
    <div class="detail-card">
      <h2 data-i18n="form.equipment_information"></h2>
      <table>
        <thead>
          <tr>
            <th data-i18n="table.equipment"></th>
            <th data-i18n="table.quantity"></th>
          </tr>
        </thead>
        <tbody>
          ${club.equipment
    ...
    <div class="detail-card">
      <h2 data-i18n="form.reorganization_information"></h2>
      <div class="info-row">
        <div><label data-i18n="form.last_reorg_date"></label><p>${club.last_reorg_date ? formatDate(club.last_reorg_date) : (window.i18n ? window.i18n.t("message.no_data") : "N/A")}</p></div>
        <div><label data-i18n="form.reorg_due_date"></label><p>${club.reorg_due_date ? formatDate(club.reorg_due_date) : (window.i18n ? window.i18n.t("message.no_data") : "N/A")}</p></div>
      </div>
      <div class="info-row">
        <div><label data-i18n="form.reorg_status"></label><p><span class="px-2 py-1 rounded text-sm ${club.reorg_status === "active" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}" data-i18n="status.${club.reorg_status}">${club.reorg_status === "active" ? (window.i18n ? window.i18n.t("status.active") : "Active") : (window.i18n ? window.i18n.t("status.expired") : "Expired")}</span></p></div>
      </div>
      <div class="no-print mt-4">
        <button onclick="openReorgModal(${club.id})" class="px-4 py-2 ${club.reorg_status === "expired" ? "bg-green-600 hover:bg-green-700" : "bg-blue-600 hover:bg-blue-700"} text-white rounded transition">
          <span data-i18n="button.add_reorg"></span>
        </button>
        ${club.last_reorg_date ? `<button onclick="deleteReorg(${club.id})" class="ml-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded transition"><span data-i18n="button.delete_reorg"></span></button>` : ""}
        <button onclick="viewReorgHistory(${club.id})" class="ml-2 px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded transition">
          <span data-i18n="button.view_history"></span>
        </button>
      </div>
    </div>
    
    <div class="print-footer" style="display: none;">
      <p data-i18n="footer.certificate_note"></p>
    </div>
    
    <div id="reorgModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <h3 class="text-xl font-bold mb-4" data-i18n="modal.add_reorg_title"></h3>
        <div class="mb-4">
          <label class="block text-sm font-medium mb-2">
            <input type="radio" name="dateOption" value="auto" checked onchange="toggleDateInput()"> 
            <span data-i18n="form.date_option_auto"></span>
          </label>
          <label class="block text-sm font-medium mb-2">
            <input type="radio" name="dateOption" value="manual" onchange="toggleDateInput()"> 
            <span data-i18n="form.date_option_manual"></span>
          </label>
        </div>
        <div class="mb-4" id="dateInputContainer" style="display:none;">
          <label class="block text-sm font-medium mb-2" data-i18n="form.reorg_date"></label>
          <input type="date" id="reorgDate" class="w-full px-3 py-2 border rounded">
        </div>
        <div class="flex justify-end gap-2">
          <button onclick="closeReorgModal()" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400" data-i18n="button.cancel"></button>
          <button onclick="saveReorg()" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700" data-i18n="button.save"></button>
        </div>
      </div>
    </div>
    ...
    <div id="historyModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 max-w-2xl w-full mx-4">
        <h3 class="text-xl font-bold mb-4" data-i18n="modal.reorg_history_title"></h3>
        <div id="historyContent" class="mb-4"></div>
        <div class="flex justify-end">
          <button onclick="closeHistoryModal()" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400" data-i18n="button.close"></button>
        </div>
      </div>
    </div>
  `;
      ...
      if (data.success) {
        ...
        if (data.data.length === 0) {
          historyContent.innerHTML =
            '<p class="text-center text-gray-500" data-i18n="message.no_reorg_history"></p>';
        } else {
          historyContent.innerHTML = `
          <table class="w-full">
            <thead>
              <tr class="bg-gray-100">
                ...
                <th class="p-2 text-left" data-i18n="form.reorg_date"></th>
              </tr>
            </thead>
            <tbody>
              ${data.data
  ...
  container.innerHTML = `
    <div class="detail-card">
      <div class="text-center text-red-600">
        ...
        <a href="dashboard.php" class="mt-4 inline-block text-blue-600 hover:text-blue-800" data-i18n="button.back"></a>
      </div>
    </div>
  `;
}
