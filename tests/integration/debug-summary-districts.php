...
list($c, $b) = req($base . '/api/summary.php');
echo "SUMMARY (truncated):\n";
$s = json_decode($b, true);
if (!$s) {
    echo "summary JSON parse error\n";
    echo $b;
    exit(1);
}
foreach (array_slice($s['data']['byDistrict'], 0, 20) as $r) {
    printf("%s => %s\n", $r['district'], $r['count']);
}

list($c, $b) = req($base . '/api/clubs-list.php?limit=1&page=1');
$one = json_decode($b, true);
$total = $one['pagination']['total'] ?? 0;
echo "\nclubs-list total clubs: $total\n";
list($c, $b) = req($base . '/api/clubs-list.php?limit=' . $total . '&page=1');
$all = json_decode($b, true);
$agg = [];
...
$summaryMap = [];
foreach ($s['data']['byDistrict'] as $r) $summaryMap[$r['district']] = (int)$r['count'];
foreach ($summaryMap as $district => $count) {
    $expected = $agg[$district] ?? 0;
    if ($expected !== $count) printf("%s: summary=%d clubs-list=%d\n", $district, $count, $expected);
}
